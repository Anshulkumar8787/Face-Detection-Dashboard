...
# Cap the longest side of any image we process to this many pixels.
# Modern phone photos are often 3000-4000px wide, which uses a huge
# amount of memory to run detection on and is unnecessary — faces are
# still easily detectable at a much smaller size. This keeps memory
# usage low and predictable, which matters a lot on free-tier hosting
# (like Render's free plan) with limited RAM.
MAX_DIMENSION = 1600
...
def detect_faces(image_path, output_path):
    ...
    # Shrink large images before doing any heavy processing.
